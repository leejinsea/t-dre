...
train_set = VGDataset(str(IMG_DIR_ROOT), str(VG_DATA_PATH), str(LOOK_UP_TABLES_PATH),
                            dataset_type='train')
...
action_describer.load_state_dict(torch.load('./model_2023_1201_5.pth'))
...
for epoch in range(STRAT_EPOCHS, END_EPOCHS):
    history = []
    epoch_loss = 0
    for itr, (_, imgs, trgs_r, trg_caps) in enumerate(train_loader):
        ...
        loss = action_describer(proposals, new_images, targets)
        ...
        print(
            f'epc: {epoch + 1}/{END_EPOCHS} | iter: {itr + 1}/{len(train_loader)} | loss: {loss.item():.8f} | time: {start.elapsed_time(end) / 1000:.2f}')
    epoch_histories.append(sum(history) / len(history)) #1: 3.08

    torch.save(action_describer.state_dict(),
               f"./model_2023_1201_{epoch + 1}.pth")